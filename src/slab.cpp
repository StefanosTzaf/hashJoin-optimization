...
void BumpAlloc::reset() {
    // reset all segments to start position
    Segment* seg = head;
    while (seg) {
        seg->cur = seg->begin;
        seg = seg->next;
...
SmallChunk* ThreadAllocatorL2::allocateSmallChunk() {
    ...
    // try to allocate data buffer for SmallChunk from level2
    void* data_ptr = level2.allocateBytes(small_chunk_bytes);
    if (!data_ptr) {
        ...
        // try again
        data_ptr = level2.allocateBytes(small_chunk_bytes);
        if (!data_ptr) {
            throw std::bad_alloc();
        }
    }
    ...
    SmallChunk* chunk = new (data_ptr) SmallChunk;
    chunk->capacity = small_chunk_bytes;
    chunk->used = 0;
    chunk->data = reinterpret_cast<std::byte*>(data_ptr);
    chunk->next = nullptr;
...
void ThreadAllocatorL2::reset() {
    // reset the bump allocator (resets cur pointers but keeps LargeChunks)
    level2.reset();
    if (current_large) {
        // give the last LargeChunk back to level2 for reuse
        level2.addSpace(current_large->data, current_large->capacity);
    }
}
...
void ThreadLocalTupleCollector::init(GlobalAllocator& level1, size_t small_chunk_bytes) {
    ...
    // new level2 allocator
    level2_ptr = new ThreadAllocatorL2(level1, small_chunk_bytes);
    counts.fill(0);
    // initialize level3 bump allocators for each partition
...
// consume function: takes a tuple and stores it in the appropriate partition
void ThreadLocalTupleCollector::consume(uint32_t part, const Tuple& tuple) {
    ...
    if (!ptr) {
        // no space, need a new segment from level2
...
SmallChunk* ThreadLocalTupleCollector::stealPartitionChunks(uint32_t part) {
    ...
    // Get the segments (chunks) for this partition from level3
    // This is used for partition exchange/merge between threads
    Segment* segments = level3[part].getSegments();
    return reinterpret_cast<SmallChunk*>(segments);
}
...
void ThreadLocalTupleCollector::reset() {
    ...
    level2_ptr->reset(); // reset L2 allocator
}